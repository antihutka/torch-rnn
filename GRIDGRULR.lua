...
local layer, parent = torch.class('nn.GRIDGRULR', 'nn.Module')

--[[
Adapted from Grid LSTM : http://arxiv.org/abs/1507.01526
--]]

function layer:__init(input_dim, hidden_dim, hidden_rank)
  parent.__init(self)

  local D, H, R = input_dim, hidden_dim, hidden_rank
  self.input_dim, self.hidden_dim, self.hidden_rank = D, H, R

  local weight_size = D * (3 * D + 3 * H) + H * 3 * D + H * H + R * 3 * H
  self.weight = torch.Tensor(weight_size)
  self.gradWeight = torch.Tensor(weight_size):zero()
  ...
  self.bottleneck = torch.Tensor() -- This will be (N, T, R)
  self.grad_bottleneck = torch.Tensor() -- This will be (N, R)
...
function layer:_get_sizes(input, gradOutput)
  ...
  local H, D, R = self.hidden_dim, self.input_dim, self.hidden_rank
  check_dims(x, {N, T, D})
  if h0 then
    check_dims(h0, {N, H})
  end
...
function layer:_split_weights(w)
  ...
  local function t(a, b)
    --print(pos .. ' .. ' .. pos + a * b .. '(' .. a .. 'x' .. b .. ') of ' .. w:size(1))
    local r = {{pos + 1, pos + a * b}}
    pos = pos + a * b
    return w[r]:view(a, b)
  end
  -- Wxt (D, 3H) 65x387
  ...
  local Wxd = t(D, 3*D)
  local Whd = t(H, 3*D)
  return Wxt, Whtg1, Whtg2, Whthc, Wxd, Whd
end
...
function layer:updateOutput(input)
  local h0, x = self:_unpack_input(input)
  local N, T, D, H , R = self:_get_sizes(input)
  ...
  local Wxt, Whtg1, Whtg2, Whthc, Wxd, Whd = self:_split_weights(self.weight)
  ...
  for t = 1, T do
    ...
    local cur_bottleneck = self.bottleneck[{{}, t}]
    cur_bottleneck:mm(prev_ht, Whtg1)
    cur_gates[{{}, {1, 2 * H}}]:addmm(cur_bottleneck, Whtg2)
    --cur_gates[{{}, {1, 2 * H}}]:addmm(prev_ht, Whtg)
    ...
    cur_gates[{{}, {2 * H + 1, 3 * H}}]:addmm(next_ht, Whthc) -- hc += Wh * r . prev_h
...
function layer:backward(input, gradOutput, scale)
  ...
  local N, T, D, H, R = self:_get_sizes(input, gradOutput)
  local Wxt, Whtg1, Whtg2, Whthc, Wxd, Whd = self:_split_weights(self.weight)
  local grad_Wxt, grad_Whtg1, grad_Whtg2, grad_Whthc, grad_Wxd, grad_Whd = self:_split_weights(self.gradWeight)
  ...
  local grad_bottleneck = self.grad_bottleneck:resize(N, R)
  ...
  for t = T, 1, -1 do
    ...
    local bottleneck = self.bottleneck[{{}, t}]
    ...
    local grad_r = grad_au:mm(grad_ahc, Whthc:t() ):cmul(prev_h)
    ...
    grad_bottleneck:mm(grad_a[{{}, {1, 2 * H}}], Whtg2:t())
    grad_Whtg1:addmm(scale, prev_h:t(), grad_bottleneck)
    grad_Whtg2:addmm(scale, bottleneck:t(), grad_a[{{}, {1, 2 * H}}])

    grad_a_sum:sum(grad_a, 1)
    grad_bt:add(scale, grad_a_sum)
    temp_buffer:cmul(prev_h, r)
    grad_Whthc:addmm(scale, temp_buffer:t(), grad_ahc)
    grad_next_h:addcmul(-1, u, grad_next_h)
    --grad_next_h:addmm(grad_a[{{}, {1, 2 * H}}], Whtg:t())
    grad_next_h:addmm(grad_bottleneck, Whtg1:t())
    temp_buffer:mm(grad_a[{{}, {2 * H + 1, 3 * H}}], Whthc:t()):cmul(r)
...
function layer:__tostring__()
  return 'nn.GRIDGRULR: ' .. self.input_dim .. 'x' .. self.hidden_dim .. ' rank ' .. self.hidden_rank
end

local swappable_tensors = { "cell", "gates", "gatesd", "bottleneck" }
