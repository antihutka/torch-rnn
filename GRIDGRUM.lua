...
local layer, parent = torch.class('nn.GRIDGRUM', 'nn.Module')

--[[
Adapted from Grid LSTM : http://arxiv.org/abs/1507.01526
--]]

function layer:__init(input_dim, hidden_dim, hidden_per_gate)
  parent.__init(self)

  local D, H, G = input_dim, hidden_dim, hidden_per_gate
  self.input_dim, self.hidden_dim, self.hidden_per_gate = D, H, G

  self.weight = torch.Tensor(D + H * G, G * H + 2 * H + 3 * D)
  self.gradWeight = torch.Tensor(D + H * G, G * H + 2 * H + 3 * D):zero()
  self.bias = torch.Tensor(G * H + 2 * H + 3 * D)
  self.gradBias = torch.Tensor(G * H + 2 * H + 3 * D):zero()
  self:reset()
...
function layer:_get_sizes(input, gradOutput)
  ...
  local H, D, G = self.hidden_dim, self.input_dim, self.hidden_per_gate
  check_dims(x, {N, T, D})
  if h0 then
    check_dims(h0, {N, H})
  end
...
function layer:updateOutput(input)
  local h0, x = self:_unpack_input(input)
  local N, T, D, H, G = self:_get_sizes(input)
  local G2 = G + 2
  local GH = G * H
  ...
  local bias_expand = self.bias:view(1, G2 * H + 3 * D):expand(N, G2 * H + 3 * D)
  local bias_expand_nt = self.bias:view(1, G2 * H + 3 * D):expand(N * T, G2 * H + 3 * D)
  local Wx = self.weight[{{1, D}}]
  local Wh = self.weight[{{D + 1, D + GH}}]
  local bias_expandt_nt = bias_expand_nt[{{},{1, G2 * H}}]
  local Wxt = Wx[{{},{1, G2 * H}}]
  local Wht = Wh[{{},{1, G2 * H}}]
  local bias_expandd = bias_expand[{{},{G2 * H + 1, G2 * H + 3 * D}}]
  local bias_expandd_b = nn.utils.addSingletonDimension(bias_expandd, 1):expand(T, N, 3 * D)
  local Wxd = Wx[{{},{G2 * H + 1, G2 * H + 3 * D}}]
  local Whd = Wh[{{},{G2 * H + 1, G2 * H + 3 * D}}]

  local h, ht = self.output, self.cell
  h:resize(N, T, D):zero()
  ht:resize(N, T, GH):zero()
  local prev_ht = h0
  self.gates:resize(N, T, G2 * H)
  self.gatesd:resize(N, T, 3 * D):copy(bias_expandd_b)

  local gates_nt = self.gates:view(N * T, G2 * H)
  local gatesd_nt = self.gatesd:view(N * T, 3 * D)
  local x_nt = x:view(N * T, D)
  local ht_nt = ht:view(N * T, GH)
  ...
  for t = 1, T do
    ...
    for g = 1, G do
      next_ht[{{}, {(g - 1) * H + 1, g * H}}]:cmul(r, prev_ht[{{}, {(g - 1) * H + 1, g * H}}]) --temporary buffer : r . prev_h
    end
    cur_gates[{{}, {2 * H + 1, G2 * H}}]:addmm(next_ht, Wht[{{}, {2 * H + 1, G2 * H}}]):tanh() -- hc += Wh * r . prev_h
    for g = 1, G do
      local hc = cur_gates[{{}, {(1 + g) * H + 1, (2 + g) * H}}] --hidden candidate : hc = tanh(Wx * x + Wh * r . prev_h + b)
      local next_htg = next_ht[{{}, {(g - 1) * H + 1, g * H}}]
      local prev_htg = prev_ht[{{}, {(g - 1) * H + 1, g * H}}]
      next_htg:addcmul(prev_htg,-1, u, prev_htg)
      next_htg:addcmul(u,hc)  --next_h = (1-u) . prev_h + u . hc
    end
    prev_ht = next_ht
  end
...
function layer:backward(input, gradOutput, scale)
  ...
  local N, T, D, H, G = self:_get_sizes(input, gradOutput)
  local G2 = G + 2
  local GH = G * H
  local Wx = self.weight[{{1, D}}]
  local Wh = self.weight[{{D + 1, D + GH}}]
  local grad_Wx = self.gradWeight[{{1, D}}]
  local grad_Wh = self.gradWeight[{{D + 1, D + GH}}]
  local grad_b = self.gradBias

  local Wxt = Wx[{{},{1, G2 * H}}]
  local Wht = Wh[{{},{1, G2 * H}}]
  local grad_Wxt = grad_Wx[{{},{1, G2 * H}}]
  local grad_Wht = grad_Wh[{{},{1, G2 * H}}]
  local grad_bt = grad_b[{{1, G2 * H}}]

  local Wxd = Wx[{{},{G2 * H + 1, G2 * H + 3 * D}}]
  local Whd = Wh[{{},{G2 * H + 1, G2 * H + 3 * D}}]
  local grad_Wxd = grad_Wx[{{},{G2 * H + 1, G2 * H + 3 * D}}]
  local grad_Whd = grad_Wh[{{},{G2 * H + 1, G2 * H + 3 * D}}]
  local grad_bd = grad_b[{{G2 * H + 1, G2 * H + 3 * D}}]

  grad_h0_tb:resize(TB, N, GH)
  grad_x:resizeAs(x):zero()
  local grad_next_h = self.buffer1:resizeAs(h0):zero()
  local temp_buffer = self.buffer2:resizeAs(h0):zero()
  local grad_a_sum = self.buffer3:resize(1,G2*H):zero()
  local temp_hg = self.buffer4:resizeAs(h0):zero()
  ...
  local grad_a = self.grad_a_buffer:resize(N, G2 * H)
  local grad_au = grad_a[{{}, {1, H}}]
  local grad_ar = grad_a[{{}, {H + 1, 2 * H}}]
  local grad_ahc = grad_a[{{}, {2 * H + 1, G2 * H}}]
  ...
  for t = T, 1, -1 do
    ...
    if TBi == TB - 1 or t == T then
      ...
      local grad_h0_tn = grad_h0_tb[{{1, TBl}}]:view(TBl*N, GH)
    ...
    local hc = self.gates[{{}, t, {2 * H + 1, G2 * H}}]

    for g = 1, G do
      grad_ahc[{{}, {(g - 1) * H + 1, g * H}}]:cmul(grad_next_h[{{}, {(g - 1) * H + 1, g * H}}], u)
    end
    tanh_gradient(grad_ahc, hc, grad_ahc)

    grad_au:zero()
    for g = 1, G do
      grad_au:addmm(grad_ahc, Wht[{{(g - 1) * H + 1, g * H}, {2 * H + 1, G2 * H}}]:t() ):cmul(prev_h[{{}, {(g - 1) * H + 1, g * H}}])
    end
    sigmoid_gradient(grad_ar, r, grad_au)

    temp_buffer:add(hc, -1, prev_h)
    for g = 1, G do
      sigmoid_gradient(temp_hg[{{}, {(g - 1) * H + 1, g * H}}], u, grad_next_h[{{}, {(g - 1) * H + 1, g * H}}])
    end
    temp_hg:cmul(temp_buffer)
    grad_au:zero()
    for g = 1, G do
      grad_au:add(temp_hg[{{}, {(g - 1) * H + 1, g * H}}])
    end
    ...
    for g = 1, G do
      temp_buffer[{{}, {(g - 1) * H + 1, g * H}}]:cmul(prev_h[{{}, {(g - 1) * H + 1, g * H}}], r)
    end
    grad_Wht[{{}, {2 * H + 1, G2 * H}}]:addmm(scale, temp_buffer:t(), grad_ahc)
    for g = 1, G do
      local gnh = grad_next_h[{{}, {(g - 1) * H + 1, g * H}}]
      gnh:addcmul(-1, u, gnh)
    end
    grad_next_h:addmm(grad_a[{{}, {1, 2 * H}}], Wht[{{}, {1, 2 * H}}]:t())
    temp_buffer:mm(grad_a[{{}, {2 * H + 1, G2 * H}}], Wht[{{}, {2 * H + 1, G2 * H}}]:t())
    for g = 1, G do
      temp_buffer[{{}, {(g - 1) * H + 1, g * H}}]:cmul(r)
    end
